...
//Stores location observables linked to the locations model.
...
var yelpAPI = function(i){
	...
	var yelp_url = locations.locations[i].yelpURL;
	...
	$.ajax(settings).done(function(results){
		...
		locations.locations[i].result = results;
...
function initMap() {
	...
	/*for (var i; i < locations.locations.length; i++){
		yelpAPI(i);
	}*/
...
ViewModel.populateInfoWindow = function(marker, infowindow, i){
	...
	if(infowindow.marker != marker){
		...
		infowindow.setContent('<h3>' + marker.title + '</h3>' + '<img src=' +
			locations.locations[i].ratingImg + '></div>' + '<div><img src=' + locations.locations[i].snippetImg +
			'></div>' + '<div>' + locations.locations[i].snippetText + '</div>');
...
var mappedData = ko.utils.arrayMap(locations.locations, function(item){
